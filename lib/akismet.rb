...
# Then modified for GRWiFi by James Stewart
#
class Akismet
  require 'net/http'
  require 'uri'
  ...
  STANDARD_HEADERS = {
    'User-Agent' => "GRWiFi/2.1 | Akismet Ruby API/1.0",
  ...
  def commentCheck(options = {})
    callAkismet('comment-check', options)
  end
  ...
  def callAkismet(akismet_function, options = {})
    ...
    Timeout.timeout(5) do
      ...
      unless data == 'true' or data == 'false' or data == '' or data == 'Thanks for making the web a better place.'
        raise "AKISMET error: #{data}"
      end
    ...
    return result
  # rescue => err
  #   logger.warn "AKISMET exception: #{err}"
  end
end