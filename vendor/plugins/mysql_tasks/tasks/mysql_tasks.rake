namespace :db do
  namespace :mysql do
    ...
    task :backup => :environment do
      ...
      cmd = "#{mysql_bin_root}/mysqldump --opt --skip-add-locks -u#{user} "
    ...
    task :restore => :environment do
      ...
      cmd = "#{mysql_bin_root}/mysql -u #{user} #{database} < #{archive}"
      puts cmd + "... [password filtered]"
      cmd += " -p'#{password}'"
      result = system(cmd)
    end
  ...
  def mysql_execute(username, password, sql)
    mysql_bin_root = "/usr/bin"
    system("#{mysql_bin_root}/mysql -u #{username} -p'#{password}' --execute=\"#{sql}\"")
  end
