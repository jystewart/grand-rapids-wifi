...
class Location < ActiveRecord::Base
  ...
  scope :for_widgets, select('name, permalink, MAX(comments_count) AS comments_count').group('name, permalink')
  ...
  class <<self
    def recent_comments(the_limit = 5)
      find_by_sql(['SELECT locations.id, locations.name, comments.blog_name AS blog_name, permalink, MAX(comments.created_at) AS last_comment_created
      FROM comments, locations
      WHERE
        comments.commentable_type = \'Location\' AND
        comments.commentable_id = locations.id AND
        comments.hide != 1
      GROUP BY locations.id, locations.name, blog_name, permalink
      ORDER BY last_comment_created DESC LIMIT ?', the_limit])
    end
